...
export class RunVideoStageUseCase {
  ...
  private async validate(video: Video): Promise<void> { const result = await this.probe.probe(this.storage.pathFor(video.storageKey)); if (result.durationSeconds > 7200) { video.failValidation("Videos must be at most 2 hours long"); await this.videos.save(video); return; } if (!result.isReadable || !result.hasSupportedCodecs) { video.failValidation("Invalid or unreadable file"); await this.videos.save(video); return; } const output = `${video.storageKey}.wav`; const extracted = await this.audio.extract(this.storage.pathFor(video.storageKey), this.storage.pathFor(output)); video.markValidated(output || extracted.audioPath); video.advanceTo("transcribing"); await this.videos.save(video); }
