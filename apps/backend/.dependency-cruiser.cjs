  ...
  forbidden: [
    ...
    },
    {
      ...
      comment:
        ...
        'Implementations are wired in main.ts. Exception: *.spec.ts files may import the ' +
        'in-memory fakes from infra/repository — per testing.md section 3, those fakes are ' +
        'themselves architectural artifacts (substitutable via LSP), not test-only mocks.',
      from: { path: '^src/usecase/', pathNot: '\\.spec\\.ts$' },
      to:   { path: '^src/infra/' },
    },
    {
    ...
    // ── Orphan modules (defensive; knip also catches these) ──────────────────
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          '^src/config/env\\.ts$',
          '^src/domain/_shared/',
          '^src/domain/health/database-health\\.gateway\\.ts$',
          '^src/usecase/[^/]+/[^/]+\\.dto\\.ts$',
          '^src/infra/http/(handler|types)\\.ts$',
    ...
    // ESLint's `no-restricted-syntax` (NewExpression) covers this — see eslint.config.mjs.
  ],
