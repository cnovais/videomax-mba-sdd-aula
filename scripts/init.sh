...
for arg in "$@"; do
    case "$arg" in
        --ignore-seeds) IGNORE_SEEDS=true ;;
        *) echo "[init] unknown flag: $arg" >&2; exit 1 ;;
    esac
done
...
ensure_postgres() {
    ...
    # Prefer the project's own docker-compose Postgres whenever docker is
    # available: `docker compose up -d` is idempotent (a no-op if the
    # container is already up), and this MUST run before the pg_isready
    # "reuse" check below — otherwise an already-running compose container
    # from a prior session would be mistaken for a native/local Postgres,
    # and `create_db_as_local_user` would try (and hang) authenticating as
    # the OS user against a container that only has the `postgres` role.
...
run_prisma() {
    [ "$HAS_DB" = "true" ] || return 0
    npm --workspace @videomax/backend run prisma:generate --silent > /dev/null
    (cd "$PROJECT_DIR/apps/backend" && DATABASE_URL="$APP_DB_URL" npx prisma migrate deploy)
}

run_seed() {
    [ "$HAS_DB" = "true" ] || return 0
    if [ "$IGNORE_SEEDS" = "true" ]; then
        log "Skipping seed (--ignore-seeds)."
        return 0
    fi
    (cd "$PROJECT_DIR/apps/backend" && DATABASE_URL="$APP_DB_URL" npx prisma db seed)
}
...
start_backend() {
    ...
    PORT="$APP_BACKEND_PORT" start_detached "$PIDS_DIR/backend.log" "$PIDS_DIR/backend.pid" \
        npm --workspace @videomax/backend run dev
}
...
start_web() {
    ...
    PORT="$APP_WEB_PORT" start_detached "$PIDS_DIR/web.log" "$PIDS_DIR/web.pid" \
        npm --workspace @videomax/web run dev
}
